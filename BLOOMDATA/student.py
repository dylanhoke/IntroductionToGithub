...
class Student:
    ...
    def __repr__(self):
        return f"Your knowledge level is: {self.knowledge}, Income is {self.income} your debt payment is: {round(self.payment,2)}"
        


class BloomTechStudent(Student):
    def __init__(self,income,debt,payment,knowledge="Some",knows_python=False,nice_job = False,):
    ...
    def debt_destroyed(self):
        ...
        self.payment = self.debt/12
        return f"Debt Destroyed: {self.debt} your payment is: {round(self.payment,2)}"
...
def rand_student(num_students=30):
    ...
    for i in range(num_students):
        ...
        students.append(BloomTechStudent(random_income,random_loan,random_payment,random_exp))
    return students
